...
class ASFF_2(nn.Module):
    def __init__(self, args, inter_dim=512, level=0, channel=[128, 256], len=[48, 24]):
        ...
        self.att = AttentionLayer(inter_dim*2, inter_dim, n_heads=args.n_heads, dropout=args.dropout)
    ...
    def forward(self, x):
        ...
        x = torch.cat((input1, input2), 1).permute(0,2,1)
        # print('input1.shape:', input1.shape)
...
class ASFF_3(nn.Module):
    def __init__(self, args, inter_dim=512, level=0, channel=[64, 128, 256], len=[48,24,12]):
        ...
        self.att = self.att = AttentionLayer(inter_dim*3, inter_dim, n_heads=args.n_heads, dropout=args.dropout)
    ...
    def forward(self, x):
        ...
        x = torch.cat((input1, input2, input3), 1).permute(0,2,1)
        out = self.att(x, x, x)
        return out.permute(0, 2, 1)
...
class ASFF_4(nn.Module):
    def __init__(self, args, inter_dim=512, level=0, channel=[16, 64, 128, 256], len=[48,24,12,6]):
        ...
        self.att = self.att = AttentionLayer(inter_dim*4, inter_dim, n_heads=args.n_heads, dropout=args.dropout)
    ...
    def forward(self, x):
        ...
        x = torch.cat((input1, input2, input3, input4), 1).permute(0,2,1)
        out = self.att(x, x, x)
